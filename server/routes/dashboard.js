...
router.post("/obtenerInfoCompletaUsuario", authorization, async (req, res) => {
  ...
  try {
    ...
    const oracleGetUserQuery =
      "BEGIN  casa_subastas.obtener_info_completa_usuario(:pcedula,:ret);  END;";
    const oracleGetUserPhonesQuery =
      "BEGIN  casa_subastas.obtener_telefonos_usuario(:pcedula,:ret);  END;";
    if (database === "pg") {
      client = await pool.connect();
      ...
      client = await oracledb.getConnection();
      user_info = await client.execute(oracleGetUserQuery, {
        pcedula: req.body.cedula,
        ret: { type: oracledb.CURSOR, dir: oracledb.BIND_OUT },
      });
      user_phone_numbers = await client.execute(oracleGetUserPhonesQuery, {
        pcedula: req.body.cedula,
        ret: { type: oracledb.CURSOR, dir: BIND_OUT },
      });
...
router.post("/getSubcategories", authorization, async (req, res) => {
  ...
  try {
    ...
    const oracleQuery =
      "BEGIN  casa_subastas.obtener_subcategorias (:ret);  END;";
    if (database === "pg") procedure = await client.query(queryText);
    else {
      const oracleProcedure = await client.execute(oracleQuery, {
        ret: { type: oracledb.CURSOR, dir: oracledb.BIND_OUT },
      ...
      while ((row = await resultSet.getRow())) {
        console.log(row);
        procedure.rows.push(new rowa(row[0], row[2], row[1]));
        //console.log(procedure)
      }
      await resultSet.close(); //Cierre de conexion y result set
    }
...
router.post("/addAuction", authorization, async (req, res) => {
  ...
  try {
    ...
    const oracleItemQuery =
      "BEGIN  casa_subastas.crear_item (:subcategoryId,:basePrice,:description,:imagen,:ret);  END;";
    const oracleAuctionQuery =
      "BEGIN  casa_subastas.crear_subasta (:itemId,:sellerAlias,:datetime,:deliveryDetails:ret);  END;";
    if (database === "pg") {
      console.log(
      ...
      const itemId = await client.execute(
        oracleItemQuery,
        {
